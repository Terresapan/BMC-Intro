...
import { Zap, Users, Calendar, Cpu, Search, Database, Server, Activity, Brain, Layers } from '@/components/cost-simulator/Icons';
import { InputSlider } from '@/components/cost-simulator/InputSlider';
import { MetricCard } from '@/components/cost-simulator/MetricCard';
import { CostPieChart, SensitivityChart } from '@/components/cost-simulator/CostCharts';
import { BreakdownBarRow } from '@/components/cost-simulator/BreakdownList';
...
export default function CostSimulatorPage() {
    ...
    return (
        <div className="min-h-screen bg-brand-dark text-slate-100 selection:bg-fuchsia-500 selection:text-white">
            <Navbar />
            <div className="container mx-auto px-4 py-24 sm:py-32">
                <div className="max-w-7xl w-full mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
                    
                    {/* LEFT COLUMN: CONTROLS */}
                    <div className="lg:col-span-4 space-y-6">
                        
                        {/* Header card with pixel-art styling */}
                        <div className="relative overflow-hidden bg-gradient-to-br from-fuchsia-500/10 via-purple-500/10 to-indigo-500/10 border-2 border-fuchsia-500/30 p-6 group hover:border-fuchsia-500/50 transition-all duration-500"
                             style={{ clipPath: 'polygon(0 0, calc(100% - 16px) 0, 100% 16px, 100% 100%, 0 100%)' }}>
                            <div className="absolute top-0 right-0 w-32 h-32 bg-fuchsia-500/20 blur-3xl -mr-16 -mt-16 opacity-50 group-hover:opacity-70 transition-opacity"></div>
                            <div className="absolute -inset-[2px] bg-gradient-to-r from-fuchsia-500 via-purple-500 to-indigo-500 opacity-0 group-hover:opacity-20 blur-lg transition-opacity duration-500"></div>
                            
                            <div className="relative z-10 flex items-center space-x-3 mb-2">
                                <div className="p-2.5 bg-gradient-to-br from-fuchsia-500 via-purple-500 to-indigo-500 text-white shadow-lg shadow-purple-500/30"
                                     style={{ clipPath: 'polygon(20% 0, 100% 0, 80% 100%, 0 100%)' }}>
                                    <Zap size={20} />
                                </div>
                                <h1 className="text-xl font-bold font-pixel tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-400 via-purple-400 to-indigo-400">
                                    Cost Simulator
                                </h1>
                            </div>
                        ...
                        <div className="bg-white/5 border-2 border-white/10 p-6 shadow-xl hover:border-white/20 transition-all duration-300"
                             style={{ clipPath: 'polygon(0 0, calc(100% - 12px) 0, 100% 12px, 100% 100%, 0 100%)' }}>
                            <h2 className="text-xs font-bold uppercase text-slate-500 tracking-wider mb-6 flex items-center font-pixel">
                                <Users size={16} className="mr-2 text-fuchsia-400" /> Business Metrics
                            </h2>
                        ...
                        <div className="bg-white/5 border-2 border-white/10 p-6 shadow-xl hover:border-white/20 transition-all duration-300"
                             style={{ clipPath: 'polygon(0 0, calc(100% - 12px) 0, 100% 12px, 100% 100%, 0 100%)' }}>
                            <h2 className="text-xs font-bold uppercase text-slate-500 tracking-wider mb-6 flex items-center font-pixel">
                                <Cpu size={16} className="mr-2 text-purple-400" /> Technical Stack
                            </h2>
                        ...
                        <div className="bg-white/5 border-2 border-white/10 p-6 shadow-xl hover:border-white/20 transition-all duration-300"
                             style={{ clipPath: 'polygon(0 0, calc(100% - 12px) 0, 100% 12px, 100% 100%, 0 100%)' }}>
                            <h2 className="text-xs font-bold uppercase text-slate-500 tracking-wider mb-6 flex items-center font-pixel">
                                <Brain size={16} className="mr-2 text-indigo-400" /> Proactive Agent
                            </h2>
                        ...
                        <div className="bg-white/5 border-2 border-white/10 p-6 shadow-xl hover:border-white/20 transition-all duration-300"
                             style={{ clipPath: 'polygon(0 0, calc(100% - 12px) 0, 100% 12px, 100% 100%, 0 100%)' }}>
                            <h2 className="text-xs font-bold uppercase text-slate-500 tracking-wider mb-6 flex items-center font-pixel">
                                <Activity size={16} className="mr-2 text-cyan-400" /> Evaluation
                            </h2>
                            <div className="flex items-center justify-between mb-4">
                                <span className="text-sm font-medium text-slate-400">Enable LLM-as-a-Judge</span>
                                <button 
                                    onClick={() => setEnableEval(!enableEval)}
                                    className={`relative w-14 h-7 transition-all duration-300 ${enableEval ? 'bg-fuchsia-500/20 border-fuchsia-500' : 'bg-slate-700/50 border-slate-600'} border-2`}
                                    style={{ clipPath: 'polygon(0 0, calc(100% - 4px) 0, 100% 4px, 100% 100%, 4px 100%, 0 calc(100% - 4px))' }}
                                >
                                    <span className={`block w-5 h-5 bg-white transform transition-transform ${enableEval ? 'translate-x-7' : 'translate-x-0.5'} mt-0.5`}
                                          style={{ clipPath: 'polygon(20% 0, 100% 0, 80% 100%, 0 100%)' }}></span>
                                </button>
                            </div>
                            {enableEval && (
                                <InputSlider 
                        ...
                        <div className="bg-white/5 border-2 border-white/10 p-6 shadow-xl hover:border-white/20 transition-all duration-300"
                             style={{ clipPath: 'polygon(0 0, calc(100% - 12px) 0, 100% 12px, 100% 100%, 0 100%)' }}>
                            <h2 className="text-xs font-bold uppercase text-slate-500 tracking-wider mb-6 flex items-center font-pixel">
                                <Server size={16} className="mr-2 text-violet-400" /> Storage & Network
                            </h2>
                            <InputSlider 
                    ...
                    <div className="lg:col-span-8 space-y-6">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            {/* Cost Structure (Pie) */}
                            <div className="bg-white/5 border-2 border-white/10 p-6 shadow-xl flex flex-col hover:border-fuchsia-500/30 transition-all duration-300"
                                 style={{ clipPath: 'polygon(0 0, calc(100% - 16px) 0, 100% 16px, 100% 100%, 0 100%)' }}>
                                <div className="mb-4">
                                    <h3 className="text-lg font-bold text-white font-pixel">Cost Structure</h3>
                            ...
                            <div className="bg-white/5 border-2 border-white/10 p-6 shadow-xl flex flex-col hover:border-purple-500/30 transition-all duration-300"
                                 style={{ clipPath: 'polygon(0 0, calc(100% - 16px) 0, 100% 16px, 100% 100%, 0 100%)' }}>
                                <div className="mb-4">
                                    <h3 className="text-lg font-bold text-white flex items-center font-pixel">
                        ...
                        <div className="bg-white/5 border-2 border-white/10 p-6 shadow-xl hover:border-indigo-500/30 transition-all duration-300"
                             style={{ clipPath: 'polygon(0 0, calc(100% - 16px) 0, 100% 16px, 100% 100%, 0 100%)' }}>
                             <div className="flex items-center justify-between mb-6">
                                 <h3 className="text-lg font-bold text-white font-pixel">Detailed Breakdown</h3>
